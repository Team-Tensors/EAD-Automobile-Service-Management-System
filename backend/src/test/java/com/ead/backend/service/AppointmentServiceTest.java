...
class AppointmentServiceTest {
    ...
    @Test
    @DisplayName("Should throw exception when cancelling someone else's appointment")
    void testCancelAppointment_AccessDenied() {
        // Arrange
        User anotherUser = new User();
        anotherUser.setId(UUID.randomUUID());
        testAppointment.setUser(anotherUser);
        ...
        assertTrue(exception.getMessage().contains("access denied") || exception.getMessage().contains("not found"));
        verify(appointmentRepository, never()).save(any());
    }
    ...
    void testGetAvailableSlotsByHour_Success() {
        // Arrange
        ...
        // Act
        Map<Integer, Integer> result = appointmentService.getAvailableSlotsByHour(serviceCenterId, testDate);
        ...
        assertTrue(result.containsKey(14));
    }

    @Test
    @DisplayName("Should throw exception when service center not found for slots")
    void testGetAvailableSlotsByHour_ServiceCenterNotFound() {
        // Arrange
        LocalDate testDate = LocalDate.now().plusDays(2);
        when(serviceCenterRepository.findById(serviceCenterId)).thenReturn(Optional.empty());

        // Act & Assert
        RuntimeException exception = assertThrows(RuntimeException.class, () -> {
            appointmentService.getAvailableSlotsByHour(serviceCenterId, testDate);
        });
        assertTrue(exception.getMessage().contains("not found"));
    }
}
